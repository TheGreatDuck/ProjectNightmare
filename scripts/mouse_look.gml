...
if (global.gamepad) //Controller
{
    ...
    if (argument0) facePitch = clamp(facePitch + (global.bind[10, 5] - global.bind[12, 6]) * 2.5, -89.95, 89.95);
}
else if (window_has_focus()) //Mouse (hard-coded, why would you want to move the camera with keys anyway?)
{
    ...
    if (argument0) facePitch = clamp(facePitch - ((display_mouse_get_y() - gH) / sensY), -89.95, 89.95);
